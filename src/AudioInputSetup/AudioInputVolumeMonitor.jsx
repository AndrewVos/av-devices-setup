...
class AudioInputVolumeMonitor extends React.Component {
  constructor(props) {
    super(props)
    this.state = { soundLevel: 0, clipping: false, pollRef: null }
  }
    ...
    if (device) {
      ...
      getSoundMeter(device.deviceId).then((meter) => {
        const ref = setInterval(() => {
          this.setState({ soundLevel: meter.volume, clipping: meter.checkClipping() })
        }, 50)
  ...
  render() {
    const { soundLevel, clipping } = this.state
    return (
