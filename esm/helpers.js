...
const getMediaDevicesList = ({
  ...
  if (type || direction) {
    return devices.then(devices => devices.filter(device => device.kind.includes(type) && device.kind.includes(direction) && device.deviceId !== 'default').reverse());
  } else {
    return devices;
  }
};
...
const validateDeviceConfig = deviceConfig => _.keys(_.pick(deviceConfig == null ? void 0 : deviceConfig.device, ['deviceId', 'kind', 'label'])).length === 3 && !!deviceConfig.constraints;
...
const getSupportedConstraints = constraints => {
  const audioInputOptions = _.pickBy(_.pick(navigator.mediaDevices.getSupportedConstraints(), AUDIO_INPUT_OPTIONS), a => a);
...
export { getPermissions, getMediaDevicesList, recordAudioToBlob, playAudioBlob, getSoundMeter, validateDeviceConfig, reduceMediaDeviceInfo, getSupportedConstraints, toTitleCase, arrayStats };